...
#define  RECEIVER_IP  "192.168.43.255"
...
{
	int itf;
	uint8_t udpbuffer[1024]; //max paket boyutu 65500 kbyte, 64000 pratik
	unsigned int n=0;
	char test[]="test";
	...
	cyw43_arch_wifi_connect_timeout_ms("G4","nano20000", CYW43_AUTH_WPA2_AES_PSK, 30000);
	upcb = udp_new();
	spcb = udp_new();
	...
	while (1) {
		...
		if(!pio_sm_is_rx_fifo_empty(pio, sm)){
			...
			while (!pio_sm_is_rx_fifo_empty(pio0, sm) && n <= BUFFER_SIZE) {
			udpbuffer[n] = uart_rx_program_getc(pio, sm);

			// mutex_enter_blocking(&UART_DATA[1].uart_mtx);
			// UART_DATA[1].uart_buffer[UART_DATA[1].uart_pos] =udpbuffer[n];
			// UART_DATA[1].uart_pos++;
			// mutex_exit(&UART_DATA[1].uart_mtx);
			n++;
			}
		}

		if(n>=1024){
		SendUDP(RECEIVER_IP,RECEIVER_PORT, udpbuffer, 1024);
		cyw43_arch_poll();
		n=0;
		}

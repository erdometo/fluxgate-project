...
def receive_data():
    ...
    buffer = []
    max_value_diff = 10000000  # max allowed difference between consecutive data values
    last_value = 0  # Keep track of last value

    while True:
        packet, addr = sock.recvfrom(1066)
        buffer += packet

        while len(buffer) >= 10:
            if len(buffer) < 10:
                continue
            if buffer[0] != 0xA5 or buffer[9] != 0xA5 :
                buffer = buffer[1:]
                continue
            channel_data = buffer[1] +  2**8*buffer[2] + 2**16*buffer[3] 
            channel_data -= 2**23 +100000  # Remove bias
            channel2_data = buffer[5] +  2**8*buffer[6] + 2**16*buffer[7] 
            channel2_data -= 2**23 +100000  # Remove bias
            
            # Ignore the value if difference from last one is too big
            if abs(channel_data - last_value) <= max_value_diff or num_samples == 0:
                last_value = channel_data
                data = np.append(data, channel_data)
                data2 = np.append(data2, channel2_data)
                num_samples += 1
            buffer = buffer[9:]  # Moving by 10 bytes as sync words are repeated after every 8 bytes
            if len(data) > 20000:
